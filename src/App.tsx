...
function App() {
    ...
    function changeTaskStatus(id: string) {  //"2"
        ...
        const nextState = tasks.map(t => t.id === id ? {...t, isDone: !t.isDone} : t) //здесь реакт увидит копию конкретной таски, это значит для реакта, что там произошли изменения и он перерисует только эту таску
        setTasks(nextState)
    }
